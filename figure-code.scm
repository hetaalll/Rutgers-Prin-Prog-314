...
(define (add-check func numrows numcols)
  (lambda (row col)
  (if (not (range-check row numrows col numcols)) #\.   ;; not in range --> print #\.
      (func row col)) )                                 ;; else call func2 with row col
)
...
;;;helper function for append to get the total column length required
(define (total-col figurea figureb)
  (+ (figure-numcols figurea) (figure-numcols figureb))
)


;;;helper function for append to get
;;;the minimun number of rows in figurea or figureb
(define (min-row figurea figureb)
...
(define (append-cols figurea figureb)
  (make-figure (lambda (row col)
                 (if( > (figure-numcols figurea) col) ((figure-func figurea) row col)
                    ((figure-func figureb) row (- col (figure-numcols figurea)))
                    ))
               (min-row figurea figureb)
               (total-col figurea figureb)
  ))


;;;helper function for append to get total row length
(define (total-row figurea figureb)
  (+ (figure-numrows figurea) (figure-numrows figureb))
 )


;;;helper function for append to get
;;;the minimun col of figurea or figureb
(define (min-col figurea figureb)
...
(define (append-rows figurea figureb)
  (make-figure (lambda (row col)
                 (if( > (figure-numrows figurea) row) ((figure-func figurea) row col)
                    ((figure-func figureb) (- row (figure-numrows figurea)) col)
                    ))
               (total-row figurea figureb)
               (min-col figurea figureb)
  ))
