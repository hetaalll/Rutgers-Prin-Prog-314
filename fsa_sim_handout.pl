...
state_struct(Name, States, state(Name, Transition, Accepts)) :- 
	member(state(Name, Transition, Accepts), States).
...
next_state_name(state(_,Transition,_), Chars, Next_name, Chars):-
	member(transition(epsilon, Next_name), Transition).

next_state_name(state(_,Transition,_), [Begin | Rest], Next_name, Rest) :- 
	member(transition(Begin, Next_name), Transition).
...
accepts(State_struct, FSA, Chars) :- 
	...
	state_struct(Next_name, FSA, NextState),
